...
export default class Board extends React.Component {
  ...
  handleCardDrop(el, target, _, sibling) {
    ...
    let movedCard = clientsList.find(client => client.id === el.dataset.id)
    movedCard = {
      ...movedCard, 
      status: targetStatus
    }
    clientsList = clientsList.filter(client => client.id !== movedCard.id)
    const siblingIndex = clientsList.findIndex(client => sibling && client.id === sibling.dataset.id)
    console.log(siblingIndex)
    clientsList.splice(siblingIndex === -1 ? clientsList.length : siblingIndex, 0, movedCard)

    const updatedClientsState = {
    ...
    this.setState({ clients: updatedClientsState })
  }
}